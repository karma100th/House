public class Rectangle {
    public static void main(String[] args) {
        ...
        //вывод результата, все условия совместил, чтобы не писать везде один sout
        if (((a + c) <= f && b <= e && d <= e) || ((a + c) <= e && b <= f && d <= f)
                || ((a + d) <= f && b <= e && c <= e) || ((a + d) <= e && b <= f && c <= f)
                || ((b + c) <= f && a <= e && d <= e) || ((b + c) <= e && a <= f && d <= f)
                || ((b + d) <= f && a <= e && c <= e) || ((b + d) <= e && a <= f && c <= f)
                || (minSum <= e && notMinSideAB <= f && notMinSideCD <= f)
